...
class SyntheticEEGDataset(Dataset):
    ...
    def process_data(self, scaling_params):
        ...
        if not self.processed:
            ...
            # Classification 0=Mauvais, 1=Moyen, 2=Excellent
            cancer_volume_z = (self.data['cancer_volume'] - mean['cancer_volume']) / std['cancer_volume']
            target_z = cancer_volume_z[:, offset:]  # (N, T)

            classes = np.zeros(target_z.shape, dtype=np.int64)
            classes[target_z > 0.0] = 1
            classes[target_z > 1.0] = 2

            outputs = classes[:, :, np.newaxis].astype(np.int64)          # (N, T, 1)
            ...
            # Add active entires
            active_entries = np.zeros((outputs.shape[0], outputs.shape[1], 1), dtype=np.float64)

            T = outputs.shape[1]

            # Update sequence lengths for offset
            new_sequence_lengths = np.minimum(sequence_lengths, T)
            new_sequence_lengths = np.clip(new_sequence_lengths, 0, None).astype(sequence_lengths.dtype)

            self.data['sequence_lengths'] = new_sequence_lengths
            sequence_lengths = new_sequence_lengths

            active_entries = np.zeros((outputs.shape[0], outputs.shape[1], 1), dtype=np.float64)
            for i in range(sequence_lengths.shape[0]):
                L = int(sequence_lengths[i])
                active_entries[i, :L, 0] = 1.0

            self.data['current_covariates'] = current_covariates
            self.data['outputs'] = outputs
            self.data['active_entries'] = active_entries

            self.data['unscaled_outputs'] = outputs
    ...
    def process_sequential_test(self, projection_horizon, encoder_r=None, save_encoder_r=False):
        ...
        if not self.processed_sequential:
            ...
            for i in range(num_patient_points):
                ...
                # How many future steps are available in outputs
                available = min(projection_horizon, outputs.shape[1] - fact_length)
                if available < 0:
                    available = 0

                # mask: 1 for available steps, 0 for padded steps
                seq2seq_active_entries[i] = np.zeros((projection_horizon, 1), dtype=np.float32)
                seq2seq_active_entries[i, :available, 0] = 1.0

                # Copy only what exists
                if available > 0:
                    seq2seq_previous_treatments[i, :available, :] = previous_treatments[i, fact_length - 1:fact_length + available - 1, :]
                    seq2seq_current_treatments[i, :available, :] = current_treatments[i, fact_length:fact_length + available, :]
                    seq2seq_outputs[i, :available, :] = outputs[i, fact_length: fact_length + available, :]

                # Sequence length is number of available steps
                seq2seq_sequence_lengths[i] = available
...
class SyntheticEEGDatasetCollection(SyntheticDatasetCollection):
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            treatment_mode: multiclass / multilabel
        """
        super(SyntheticEEGDatasetCollection, self).__init__()
        self.seed = seed
        np.random.seed(seed)

        self.train_f = SyntheticEEGDataset(chemo_coeff, radio_coeff, num_patients['train'], window_size, max_seq_length,
                                              'train', lag=lag, treatment_mode=treatment_mode)
        self.val_f = SyntheticEEGDataset(chemo_coeff, radio_coeff, num_patients['val'], window_size, max_seq_length, 'val',
                                            lag=lag, treatment_mode=treatment_mode)
        self.test_cf_one_step = SyntheticEEGDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                       max_seq_length, 'test', mode='counterfactual_one_step', lag=lag,
                                                       treatment_mode=treatment_mode)
        self.test_cf_treatment_seq = SyntheticEEGDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
